...
class Login extends Model//打造验证继承类
{
 	public function login($username,$password){//打造登录验证函数
       $user=Db::table('user')->where('username','=',$username)->find();//判断用户名是否存在
     	if ($user) {
     		if ($user['password']==MD5($password)) {//判断密码是否正确
     			\think\Session::set('id',$user['gl_id']);//将ID存储到session中
		    	\think\Session::set('username',$user['username']);//将用户名存储到session中
     			return 1;//成功登录
     		}
     		else{
